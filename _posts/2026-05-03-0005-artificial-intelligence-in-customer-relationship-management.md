...
- 25명의 C-level·전문가 인터뷰를 Gioia 방법론으로 코딩해 <strong>AI를 CRM에 성공적으로 통합하는 13 step·4 macro-phase 프레임워크</strong>를 도출. SugarCRM·Microsoft Dynamics 컨설턴트 검증까지 포함.
- 기존 AI 통합 프레임워크가 strategy → data → model → scale 의 선형 모델인 반면, 이 논문은 **CRM 특유의 cross-functional·iterative한 성격** 때문에 ethics committee, customer data centralization, people learning이 <strong>첫 단계부터 병행</strong>되어야 한다고 본다.
...
- **Cultural foundation·strategic alignment.** 기존: 통합 후반부에 문화 변화 (*Herremans, 2021*). 본 논문: customer-focused culture를 통합 시작 <em>전</em>에 박아야 한다.
