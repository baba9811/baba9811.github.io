...
핵심은 **분모의 reference policy 에는 short context $x_S$ 를 넣는다**는 점이다. 일반 DPO 에서 reference 가 long context 에서 OOD 로 동작해 점수가 무너지는 문제를 우회한다. $\beta = 0.1$, $\lambda = 0.01$ — preference 의 영향력은 작게 잡고 NLL 이 주된 driver 다.
...
ProLong 텍스트 데이터를 **빼고** Mistral 에 시각 LC 데이터만으로 10B 토큰 CPT — HELMET 이 37 → 48.5 로 상승한다. 즉 CPT 의 HELMET 향상이 단순히 섞은 텍스트 데이터 덕분이 아니라 **시각 LC 학습 자체의 transfer 효과**라는 뜻이다.
