...
- **Reinforcement Learning (RL).** Reward-driven optimization over ranked sessions, modeling non-differentiable metrics. Objective: $-\left[ r\_\phi(x, y^+) - \beta D\_{\text{KL}}(\pi\_\theta(y \mid x) \,\Vert \, \pi\_{\text{ref}}(y \mid x)) \right]$ (LEA, RPP). It needs large-scale feedback and can be unstable.
